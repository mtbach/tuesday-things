...
echo "$($PSQL "SELECT DISTINCT name FROM games AS a FULL JOIN teams ON a.winner_id=teams.team_id FULL JOIN games AS b ON teams.team_id=b.opponent_id WHERE a.year=2014 AND a.round='Eighth-Final' OR (b.year=2014 AND b.round='Eighth-Final') ORDER BY name")"
